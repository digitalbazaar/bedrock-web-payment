...
import {Payment} from 'bedrock-payment/lib/Payment.js';
 ...
 *   the payment api.
 * @param {object} [options.httpsAgent = axios] - The module used to
 *   fetch from the api.
 *
 * @returns {PaymentService} The PaymentService.
 */
export class PaymentService {
  constructor({baseURL = '/payment', httpsAgent = axios} = {}) {
  ...
  async processPayment({order, payment}) {
    ...
    const {data} = await this.httpsAgent.post(url, {order, payment});
    return data;
  }
}
